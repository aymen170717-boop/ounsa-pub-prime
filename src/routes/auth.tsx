...
export const Route = createFileRoute("/auth")({
  head: () => ({ meta: [{ title: "Connexion — Ounsa PUB ERP" }] }),
  component: AuthPage,
});
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="absolute end-4 top-4 flex gap-2">
        <button onClick={() => setLang("fr")} className={`rounded-md px-3 py-1 text-xs ${lang === "fr" ? "bg-primary text-primary-foreground" : "bg-muted"}`}>FR</button>
        <button onClick={() => setLang("ar")} className={`rounded-md px-3 py-1 text-xs ${lang === "ar" ? "bg-primary text-primary-foreground" : "bg-muted"}`}>عربي</button>
      </div>

      <div className="w-full max-w-md">
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-white p-3 shadow-lg">
            <img src={logo} alt="Ounsa PUB" className="h-full w-full object-contain" />
          </div>
          <h1 className="text-2xl font-bold">{t("app_name")}</h1>
          <p className="mt-1 text-xs text-muted-foreground">{t("app_tagline")}</p>
        </div>
          ...
          <Tabs defaultValue="login">
            ...
            <TabsContent value="signup" className="space-y-4 pt-4">
              <form onSubmit={onSignup} className="space-y-3">
                ...
                <Button type="submit" disabled={loading} className="w-full">{t("signup")}</Button>
              </form>
            </TabsContent>
          </Tabs>
